...
public class RBC3BlockBean {
    ...
    public static String[] order = {"PID","Index","Local_ID","Remote_ID","Protocol_Version","Configure_Version","Send_Count","ZNQD_Count","BSFX_Count","BSFQ_Count","JLXX_Count","Send_Codes"};
    ...
    public void setAll(InterfaceFileBean oneBlock) {
        ...
        if (!global.containsKey("PID")) {
            ...
            SwitchInterfaceBlock.switchInfo.get("RBC3").put(global.get("INDEX").getValue(), new ArrayList<String>(){{this.add(StringUtil.decToHex(String.valueOf(pid), 8));}});
            this.PID.setValue(StringUtil.decToHex(String.valueOf(pid), 8));
        } else {
            SwitchInterfaceBlock.switchInfo.get("RBC3").put(global.get("INDEX").getValue(), new ArrayList<String>(){{this.add(StringUtil.decToHex(global.get("PID").getValue(), 8));}});
        ...
        for (String globalKey : global.keySet()) {
            ...
            if (globalKey.equals("SWITCH_WEIGHT")) {
                SwitchInterfaceBlock.switchInfo.get("RBC3").get(global.get("INDEX").getValue()).add(StringUtil.decToHex(value, 8));
            }
        }
